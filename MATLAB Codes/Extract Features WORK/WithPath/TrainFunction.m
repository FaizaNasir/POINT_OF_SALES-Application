function Training = TrainFunction(path,N1,alpha,epoch)
load (strcat(path,'\','INPUT_FEATURES.mat')); %importing Input Features
load(strcat(path,'\','OUTPUTS.mat')); %importing Actual Outputs
...
maxVec = max(INPUT_FEATURES); %max of each column in IPUTs
...
save (strcat(path,'\','INPUT_FEATURES_Norm.mat'),'INPUT_FEATURES');
...
save (strcat(path,'\','maxVec.mat'),'maxVec');
save (strcat(path,'\','minVec.mat'),'minVec')
save (strcat(path,'\','weight1.mat'),'w1')
save (strcat(path,'\','weight2.mat'),'w2')
save (strcat(path,'\','Biased1.mat'),'b1')
save (strcat(path,'\','Biased2.mat'),'b2')
save (strcat(path,'\','path.mat'), 'path')
